...
Installation:
   Just copy the Shadow.PlusMod folder to:
        %appdata%\..\Local\ProjectorGames\FortressCraft\Mods directory  
...
Conveyor Filter Plus Mk1:
   ...
   Useful for creating Filtering chains with a Category/Not Category using a 
       Storage Hopper and 2 Conveyor Filters Plus.
   ...
   Known Issues:
      ...
      I implemented my own StorageHopper routines for Counting/Deleting Inventory:
         (Some properties and methods that I needed in StorageHopper are private 
          No RoundRobin deletion).
     To update the StorageHopper's Text Panel I call ToggleHopper twice:
        (forces a Text update flag value to set along with some other cleanup 
         without changing the original vacuum status).

     When the New StorageHopper API's are available I plan on rewriting parts of the
...
AutoBuilder Plus Mk1-Mk4:
   ...
   Replacement option: Blocks will only be replaced when 'ON' 
       (ORES/OBJECTS/Entities are never Replaced)
   ...
   Known Issues:    
      Server Support is not implemented.
      AutoBuilder Mk1-Mk4: Unable to change the color of the entire machine
         (only the shuttle is a different color).
      AutoBuilder Mk1-Mk4: Unable to Hide Research until the AutoBuilder has been
         built and Scanned
...
Download:
    https://www.dropbox.com/sh/urz39vwm2asef0p/AAAsndeo4cXmdmlrKDP1xSbqa?dl=0